...
public class PushWorker : BackgroundService
{
        ...
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var processed = await ProcessPushNotificationWithRetryAsync(consumeResult.Message.Value, stoppingToken);
    ...
    private async Task<bool> ProcessPushNotificationWithRetryAsync(string messageValue, CancellationToken cancellationToken)
    {
        for (var attempt = 1; attempt <= _maxProcessingAttempts; attempt++)
        {
            var processed = await ProcessPushNotification(messageValue, cancellationToken);
            if (processed)
            {
                return true;
            }
            ...
            {
                _logger.LogWarning("[PUSH] Reintento {Attempt}/{MaxAttempts} de procesamiento", attempt + 1, _maxProcessingAttempts);
    ...
    private async Task<bool> PublishToDlqWithRetryAsync(ConsumeResult<string, string> consumeResult, CancellationToken cancellationToken)
    {
        for (var attempt = 1; attempt <= _maxDlqPublishAttempts; attempt++)
        {
            ...
            {
                _logger.LogWarning("[PUSH] Reintento {Attempt}/{MaxAttempts} para publicar en DLQ", attempt + 1, _maxDlqPublishAttempts);
                await Task.Delay(_retryBackoff, cancellationToken);
            }
        }

        _logger.LogError("[PUSH] Se agotaron {MaxAttempts} intentos para publicar en DLQ. El offset no se commiteará y se reintentará en el siguiente ciclo.", _maxDlqPublishAttempts);
        return false;
    }
